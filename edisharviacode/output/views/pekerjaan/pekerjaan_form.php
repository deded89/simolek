<div class='row'>
	<div class='col-xs-12'>
	    <div class='box box-primary'>
			<form action="<?php echo $action; ?>" method="post">
				<table class='table table-bordered'>		
		
					<tr>
						<td><label for="varchar">Nama <?php echo form_error('nama') ?></label></td>
						<td><input type="text" class="form-control" name="nama" id="nama" placeholder="Nama" value="<?php echo $nama; ?>" /></td>    
					</tr>
            
					<tr>
						<td><label for="varchar">Kegiatan <?php echo form_error('kegiatan') ?></label></td>
						<td><input type="text" class="form-control" name="kegiatan" id="kegiatan" placeholder="Kegiatan" value="<?php echo $kegiatan; ?>" /></td>    
					</tr>
            
					<tr>
						<td><label for="mediumint">Skpd <?php echo form_error('skpd') ?></label></td>
						<td><input type="text" class="form-control" name="skpd" id="skpd" placeholder="Skpd" value="<?php echo $skpd; ?>" /></td>    
					</tr>
            
					<tr>
						<td><label for="tinyint">Jenis <?php echo form_error('jenis') ?></label></td>
						<td><input type="text" class="form-control" name="jenis" id="jenis" placeholder="Jenis" value="<?php echo $jenis; ?>" /></td>    
					</tr>
            
					<tr>
						<td><label for="tinyint">Metode <?php echo form_error('metode') ?></label></td>
						<td><input type="text" class="form-control" name="metode" id="metode" placeholder="Metode" value="<?php echo $metode; ?>" /></td>    
					</tr>
            
					<tr>
						<td><label for="decimal">Pagu <?php echo form_error('pagu') ?></label></td>
						<td><input type="text" class="form-control" name="pagu" id="pagu" placeholder="Pagu" value="<?php echo $pagu; ?>" /></td>    
					</tr>
            
					<tr>
						<td><label for="decimal">Realisasi <?php echo form_error('realisasi') ?></label></td>
						<td><input type="text" class="form-control" name="realisasi" id="realisasi" placeholder="Realisasi" value="<?php echo $realisasi; ?>" /></td>    
					</tr>
            
					<tr>
						<td><label for="int">Kontrak <?php echo form_error('kontrak') ?></label></td>
						<td><input type="text" class="form-control" name="kontrak" id="kontrak" placeholder="Kontrak" value="<?php echo $kontrak; ?>" /></td>    
					</tr>
            
					<tr>
						<td><label for="int">Serah Terima <?php echo form_error('serah_terima') ?></label></td>
						<td><input type="text" class="form-control" name="serah_terima" id="serah_terima" placeholder="Serah Terima" value="<?php echo $serah_terima; ?>" /></td>    
					</tr>
            
					<input type="hidden" name="id" value="<?php echo $id; ?>" /> 
					<tr>
						<td colspan='2'>
							<button type="submit" class="btn btn-primary"><?php echo $button ?></button> 
							<a href="<?php echo site_url('Pekerjaan') ?>" class="btn btn-danger">Cancel</a>
						</td>	
					</tr>
				</table>
			</form>
		</div>
	</div>
</div>
	
<!-- jQuery 2.2.3 -->
<script src="<?php echo base_url();?>assets/plugins/jQuery/jquery-2.2.3.min.js"></script>
<script type="text/javascript">
	$(document).ready(function () {  		
		$("#serah_terima").focus();
	});
</script>
