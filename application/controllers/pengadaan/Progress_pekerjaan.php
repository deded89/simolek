...
class Progress_pekerjaan extends CI_Controller
{
  ...
  public function create_action(){
    ...
    if ($this->form_validation->run() == FALSE) {
      ...
      //CEK KELENGKAPAN TAHAPAN PROGRESS
      $this->cek_kelengkapan_progress($this->input->post('id_p',TRUE),$this->input->post('progress',TRUE));
  ...
  public function cek_kelengkapan_progress($id_p, $progress_input){
    ...
    if ($p > 1 and $p < 5){
      for ($i = 1; $i < $p; $i++) {
        ...
        if (!$exist){
          $this->session->set_flashdata('error', 'Mohon Input Tahapan Progress '.$progress_data->nama.' terlebih dahulu');
          redirect(site_url('pengadaan/pekerjaan/read/'.$id_p));
        }
      }
    }
